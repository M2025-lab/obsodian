...
1. **派单待确认列表**：工地页新增二级 tab「派单待确认」，支持「待上级确认 / 待工人确认 / 问题待审核 / 约工驳回待审核 / 待整改 / 额外项提报」等筛选。
2. **派单二级确认**：安装/拆除/橱柜派单由「待接单」拆为「待上级确认(2001)」「待工人确认(2002)」，拒单可重派。
3. **上级查询改造**：问题反馈、约工驳回、整改费用/请假审批改为查「长期合作项目经理」。
4. **问题费用审批**：一级审批=安装项目经理；二级审批=COE；费用超额查 COE 直属上级。
5. **安装/拆除项目经理存储→结算**：cube 需落库长期合作项目经理 id/name。
6. **施工包进展**：在作业中心展示「待上级确认/待工人确认」及操作人/原因。
7. **施工包卡片工程量确认标签**（依赖第三方接口，8/8 交付，未就绪）。
...
- `AttachInfoApi#listAttachInfo` —— 安装/拆除工人查上级（`AttachInfoManager#listAttachInfo`，约工驳回提交处调用）。
...
- ⏳ 3.2.10 定时刷「拆建模式」任务（逻辑待定）。
- ⏳ 3.2.11 结算侧其余改造清单/排期（待定）。
...
- 服务者中心长期合作关系数据质量（缺失/过期/错误）→ 开城前存数据核对 + 关系准确性监控。
