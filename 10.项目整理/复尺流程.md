...
> **复尺的触发事件**与**内部创建机制**是两套互补的维度，组合起来才是完整视图。
...
测量申请单事件触发后，**模式选择**是决定"走哪个内部机制"的第一道关卡：

```java
// 三个判定条件
isMaterialSchedule   = configQueryService.materialScheduleSwitch(projectOrderId)
isDownServiceOrder   = !isBeijing || beijingServiceOrderDownSwitch
isNewV2_5            = processVersion=="V2_5" && businessModel==BEI_WO && 非北京分公司
```

```
                    ┌─────────────────────┐
                    │ isMaterialSchedule   │  材料进排程开城？
                    └──────────┬──────────┘
                               │
               false           │           true
         ┌─────────────────────┴─────────────────────┐
         │  createTaskOld(param)                     │  跳过 createTaskOld
         │  → 仅生成 ORDER 任务(旧逻辑,无复尺)        │
         └─────────────────────┬─────────────────────┘
                               │
                    ┌──────────▼──────────┐
                    │ isDownServiceOrder   │  下服务单开城？
                    └──────────┬──────────┘
                               │
               false           │           true
         ┌─────────────────────┴─────────────────────┐
         ...
         └───────────────────────────────────────────┘
              ...
              └────────┬──┘      └─────┬──────────────┘
                       │               │
              ┌────────▼───────────────▼──┐
              │ materialScheduleSwitch     │  再次判断(值同
              │ (与 isMaterialSchedule 相同)│  isMaterialSchedule)
              └──┬─────────────────────┬──┘
                 │                     │
            false│                     │true
         ┌───────▼──┐           ┌──────▼──────────┐
         ...
         └───────┬──┘           │ 排程材料配置      │
              ...
              ┌─────────▼───────────────┐
              │ createMaterialTask       │
              │ (execCreateMaterialProcess) │
              │                          │
              │ ┌ 模板配置查任务类型      │
              │ ├ 规则解析 nodeProcessName│
              │ └ 定额复尺 9999999       │
              │ → 决定最终生成哪些任务    │
              └─────────────────────────┘
```

**关键代码**（`ScmMeasureApplyServiceImpl.java:107-161`）：
```java
if (!isMaterialSchedule) { createTaskOld(param, false); }    // 旧逻辑
if (!isDownServiceOrder) { return; }                         // 不下服务单→结束

context.setSupportMode(ModeEnum.HOME2_5);                     // 默认 mode=5
if (projectOrderUtil.isNewV2_5(...)) {                        // 新2.5→mode=6
    context.setSupportMode(ModeEnum.HOME2_5_MANPOWER);
}
if (configQueryService.materialScheduleSwitch(...)) {          // 排程开关→mode=7
    context.setSupportMode(ModeEnum.DELIVERY_FLOW);
}
```
...
项目变更完成/手动再次复尺，判定条件一致：

```java
judgeNewFuchiProcess(projectOrder) == true   // 新复尺开关开启
&& 物料不在黑名单中
&& (路径②)存在已完成的复尺 + 执行人是供应商
&& (路径③)不存在未完成的复尺(判重)
→ 调用 newFuchiMaterialTaskCreate() 直接创建 node=60 的复尺任务
```

### 维度A-④ 的判定逻辑

```java
// ServiceOrderServiceImpl.completeNodeByServiceOrder()
fulfillmentLinkCode == RECHECK_SCALE.fulfillmentLinkCode (5)

switch (event) {
    case SUBMIT_RECHECK_SCALE_RESULT → 完成 node 60，合格
    case SERVICE_COMPLETED           → 完成 node 60，合格
    case HANG_UP                     → 完成 node 60，不合格(触发重启)
}
```
...
flowchart TD
    subgraph TRIGGER["触发事件（外部）"]
        direction LR
        T1["① 测量申请单事件<br/>measure-apply-order"]
        T2["② 项目变更完成"]
        T3["③ 手动再次复尺"]
        T4["④ 服务单状态变更"]
    end
    ...
    T_SWITCH -->|"② 项目变更完成"| AGAIN_CHECK1
    T_SWITCH -->|"③ 手动再次复尺"| AGAIN_CHECK2
    T_SWITCH -->|"④ 服务单状态变更"| SERVICE_COMPLETE
    T_SWITCH -->|"① 测量申请单事件"| MODE_DECISION

    subgraph AGAIN_CHECK1["再次复尺-变更(②)"]
        AC1["judgeNewFuchiProcess开启?"]
        AC1 -->|"是"| AC2["物料不在黑名单?"]
        AC2 -->|"是"| AC3["已存在完成的复尺?"]
        AC3 -->|"是"| AC4["执行人是供应商?"]
        AC4 -->|"是"| NEW_FUCHI
    ...
    subgraph AGAIN_CHECK2["再次复尺-手动(③)"]
        AM1["judgeNewFuchiProcess开启?"]
        AM1 -->|"是"| AM2["物料不在黑名单?"]
        AM2 -->|"是"| AM3["无未完成的复尺?"]
        AM3 -->|"是"| AM4["旧复尺执行人是供应商?"]
        AM4 -->|"是"| NEW_FUCHI
    end

    NEW_FUCHI["newFuchiMaterialTaskCreate<br/>直接创建 node=60 复尺任务"] --> RUNNING

    subgraph SERVICE_COMPLETE["服务单完成节点(④)"]
        SC1{"fulfillmentLinkCode<br/>== RECHECK_SCALE(5)?"}
        SC1 -->|"是"| SC2{"事件类型?"}
        SC2 -->|"待验收/已完成"| SC3["完成 node 60 (合格)"]
        SC2 -->|"挂起"| SC4["完成 node 60 (不合格/重启)"]
    end

    SC3 --> RUNNING
    SC4 --> RUNNING

    subgraph MODE_DECISION["测量申请单 → 模式判定"]
        direction TB
        MD1{"isMaterialSchedule<br/>(材料进排程)?"}
        MD1 -->|"否"| MD2["createTaskOld<br/>仅生成ORDER(旧逻辑)"]
        MD1 -->|"是"| MD3["跳过旧逻辑"]
        MD2 --> MD4{"isDownServiceOrder<br/>(下服务单)?"}
        MD3 --> MD4
        MD4 -->|"否"| MD5["return 结束<br/>不下服务单不生成复尺"]
        MD4 -->|"是"| MD6{"isNewV2_5?<br/>(processVersion+"<br/>+businessModel)"}
        MD6 -->|"否"| MD7["默认 mode=5<br/>HOME2.5"]
        MD6 -->|"是"| MD8["mode=6<br/>HOME2.5用工管理"]
        MD7 --> MD9{"materialScheduleSwitch<br/>(跟isMaterialSchedule同值)"}
        MD8 --> MD9
        MD9 -->|"否→保持mode"| MD10
        MD9 -->|"是→mode=7 DELIVERY_FLOW"| MD10["createMaterialTask<br/>execCreateMaterialProcess"]
    end

    MD10 --> CREATE_INNER

    subgraph CREATE_INNER["内部创建机制（根据模板/规则）"]
        direction TB
        CI1["配置查询: queryAllMaterialForm"]
        CI2["规则解析: nodeProcessName<br/>'测量-设计-复尺-下单…'"]
        CI3["定额复尺: supplierCode=9999999<br/>只生成复尺任务"]
        CI4["零售: createMaterialTaskWithDefaultParam<br/>直接创建并完成 node 60"]
    end
    ...
    CI1 --> CI_TASK{"模板中是否包含<br/>RECHECK_SCALE?"}
    CI_TASK -->|"是"| CI_TASK_CREATE["创建复尺 Dispatch"]
    CI_TASK -->|"否"| END
    CI2 --> CI_TASK
    CI3 --> CI_TASK_CREATE

    CI_TASK_CREATE --> SAVE["saveTaskDispatch / buildAndSaveTaskDispatchNode"]
    SAVE --> NODE_20

    subgraph NODE_20["通知复尺 (20节点)"]
        direction LR
        N201["填写期望上门时间"]
        N202["handleNode 完成20节点"]
        N203["激活下一节点(60)"]
    end

    NODE_20 --> NODE_60

    subgraph NODE_60["复尺执行 (60节点)"]
        direction LR
        N601["获取复尺模板"]
        N602["查看上次测量数据(可选)"]
        N603["提交复尺测量数据"]
    ...
    NODE_60 --> QUALIFY{"合格判定"}

    QUALIFY -->|"合格(1)"| PASS_PATH
    QUALIFY -->|"不合格(2)"| REJECT_PATH

    subgraph PASS_PATH["合格路径"]
        direction LR
        P1["完成当前节点"]
        P2["更新任务状态 FINISHED"]
        P3["同步OMS/VSS"]
    ...
    PASS_PATH --> DOWNSTREAM

    subgraph REJECT_PATH["驳回路径"]
        direction LR
        R1["驳回重启"]
        R2["restart +1"]
        R3["新建20+60节点"]
    ...
    REJECT_PATH -.->|"回到通知复尺"| NODE_20

    subgraph DOWNSTREAM["下游任务激活"]
        direction LR
        D1["下单任务 (ORDER)"]
        D2["报价变更 (DESIGN_REVIEW)"]
        D3["新复尺流程额外步骤:"]
        D4["用量确认"]
        D5["SKU信息保存"]
        D6["invokeOrder下单"]
    ...
    subgraph RUNNING["任务运行中…"]
        style RUNNING fill:#f5f5f5,stroke:#999
    end

    subgraph END["结束"]
        style END fill:#f5f5f5,stroke:#999
    end
```
...
- [[10.项目整理/安装流程]] — 安装任务节点流转详析（对比查看）
- [[10.项目整理/主材全链路业务与系统配置总结]] — 全链路业务与系统配置总览
- [[10.项目整理/枚举类和上下游梳理-starlord系统]] — 系统职责与枚举定义
- [[10.项目整理/全部枚举类型梳理]] — 全量枚举类型列表
- [[10.项目整理/接口逻辑梳理/测量申请单事件处理（ScmMeasureApplyEventHandler.handleBiz）业务流梳理]] — 测量申请单事件驱动处理
