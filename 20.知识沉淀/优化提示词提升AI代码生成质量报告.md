# 优化提示词对 AI 代码生成质量的提升报告

> 模型 deepseek-v4-flash｜任务 零售切排程迁移（纯零售为重点）｜日期 2026-09-10 核心结论：结构化提示词模板使 AI 沟通轮数 8→2，返工率 ~34%→~16%。

## 0、适用

适用于入口明确 + 多条件分流 + 上下游联动的迁移/改造的功能
...
- **对照**：同一任务（零售切排程迁移）在两个分支上分别用优化前/后提示词实践，任务复杂度对等。优化前 8 轮/~34%，优化后 2 轮/~16%。优化后修改范围集中在bo包的位置，字段判空的位置，提示词未声明的地方（人工梳理时遗漏的点）
...
### 7.1 下一步验证
- 结合 [Grill-me](https://github.com/TimothyVang/Grill-me) 的 grilling-me skill，验证其帮助补全模板中信息的效果。

### 7.2 现在的问题
1. 需要人给的信息太多，工作量过大，无法体现 AI 提效。
2. 对一段话的理解，人与 AI 存在差异（如现象 1）
3. **不够通用**。目前仅根据单个需求测试，测试集不足；先尝试构建更通用的方法，使用过程中再优化。
...
| 方案            | 做法                                        | 问题                               |
| ------------- | ----------------------------------------- | -------------------------------- |
| 差分对拍          | 同一批输入分别跑旧逻辑和新逻辑，diff 对比                   | 所需输入过多，需模仿全部场景；目前无自动化方案，难以覆盖全部场景 |
| 澄清回环          | 让 AI 复述对需求的理解、列出假设并标出歧义，人工审核；可解决 AI 的静默误解 | 需人仔细审核辨别，略有成本，可接受                |
| AI 从代码取信息（主要） | 区分哪些信息 AI 可取到，让 AI 能取到的信息从代码中取            | 可能取值不对造成错误理解（如之前取单据类型取到了错误的方法）   |
|               |                                           |                                  |

**问题二：理解差异**

1. **结构化输入**：打平人与 AI 的理解差异。
2. **YAML 结构化校验**：对 AI 输出的代码结果进行校验，提高代码质量。
